...
class Form extends Template
{
    ...
    public function getAttributeInputType($attribute)
    {
        ...
        if ($imputType == 'select' || $imputType == 'multiselect' || $imputType == 'radio') {
            return 'select';
        }
	...
	public function getAttributeRadioElement($attribute)
    {
        $extra = '';
        $options = $attribute->getSource()->getAllOptions(false);
...
print_r($options);die; */
       ...
       /*  if (is_array($options) && count($options) > 2) {
		...
		$optionImg = $objectManager->get('Magento\Store\Model\StoreManagerInterface')
                    ->getStore()
		...
		{	
			if($option['value']){
				$val = $option['value'];
				$label = $option['label'];
				$labelnew = str_replace(' ', '-', strtolower($label));
				$custom .= '<input class="newclass" '.$labelnew.'  type="radio" name="'.$name.'" value="'.$val.'" id="'.$val.'" validate="required:true" onclick="myradio();"><label for="'.$val.'"> <img src="'.$optionImg.$labelnew.'.png"/>'.$label .'</label><br>';
